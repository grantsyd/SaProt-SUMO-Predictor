# -*- coding: utf-8 -*-
"""
Extract_SaProt_Features_Dataset2.py

Extract SaProt representations for Dataset2 41-mer candidate sequences.
...
Output:
    features/dataset2_saprot_<pooling>.npy
...
def pool_hidden(hidden: torch.Tensor, mode: str) -> np.ndarray:
...
def extract_saprot_features(sequences, model_path, pooling, batch_size, device):
    ...
    with torch.no_grad():
        for start in tqdm(range(0, len(sequences), batch_size), desc=f"SaProt-{pooling}"):
            ...
            features = pool_hidden(hidden, pooling)
...
def main():
    ...
    X = extract_saprot_features(args.sequences if hasattr(args, 'sequences') else sequences,
                                args.model, args.pooling, args.batch_size, args.device)

    out_file = out_dir / f"dataset2_saprot_{args.pooling}.npy"
    np.save(out_file, X)
